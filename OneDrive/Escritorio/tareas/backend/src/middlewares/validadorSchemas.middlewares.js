...
export const validarSchema = (schema) => (req, res, next) => {
  try {
    ...
    return res.status(400).json(error.errors.map((error) => error.message)); //aca este error va a recorrer el mensaje que tiene el objeto de error para que me simplifique este error
  }
};
